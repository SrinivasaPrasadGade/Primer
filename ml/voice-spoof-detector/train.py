...
This yields the 128x400 input NoteAuthNet's LCNN expects.
...
def to_mel_spectrogram(y: np.ndarray) -> np.ndarray:
...
class ASVspoofDataset(Dataset):
    ...
    def __getitem__(self, idx):
        y, sr = sf.read(self.filepaths[idx], dtype="float32")
        if sr != SAMPLE_RATE:
            y = librosa.resample(y, orig_sr=sr, target_sr=SAMPLE_RATE)
        y = fix_length(y, CLIP_SAMPLES, self.train, self.rng)
        mel = to_mel_spectrogram(y)
...
def run_epoch(model, loader, weights: dict, device, optimizer=None):
...
def main():
    ...
    weights = {k: torch.tensor(v, dtype=torch.float32) for k, v in class_weights(train_df["label"].to_numpy()).items()}

    train_loader = DataLoader(ASVspoofDataset(train_df, train=True), batch_size=args.batch_size,
                               shuffle=True, num_workers=args.num_workers)
    dev_loader = DataLoader(ASVspoofDataset(dev_df, train=False), batch_size=args.batch_size,
    ...
    for epoch in range(1, args.epochs + 1):
        train_loss, train_auc, train_eer = run_epoch(model, train_loader, weights, device, optimizer)
        dev_loss, dev_auc, dev_eer = run_epoch(model, dev_loader, weights, device)
    ...
    eval_loader = DataLoader(ASVspoofDataset(eval_df, train=False), batch_size=args.batch_size,
                              shuffle=False, num_workers=args.num_workers)
    eval_loss, eval_auc, eval_eer = run_epoch(model, eval_loader, weights, device)
